...
import useChecks from '../ClassesPanel/hooks/useChecks';
import { computeHeightAndMargins } from '@fullcalendar/common';
    ...
    const handleMultipleSelectMouseUp = (event: KonvaEventObject<WheelEvent>) => {
        if (selectArea.length === 1) {
            ...
            if (xValid && yValid) { 
              classes.forEach(({annotations, name})=>{
                  annotations.forEach(({id, classId, shapes})=>{
                    ...
                    if(shapeData.type == "Line"){
                      console.log(width, height , bgWidthScale, bgHeightScale)
                      // x-width<shapeData.points[0] && y-height<shapeData.points[1] this could also be additional condition to cover whole line
                      if( x-width<shapeData.points[0] && y-height<shapeData.points[1] && x > shapeData.points[2] && y > shapeData.points[3]){
                        multiselectedAnnotatorsArray.push({id:id, classId: classId, shapeId:shapeData.id, className : name })
                        multiselectShapeIds.push(shapeData.id) 
              ...
              console.log(multiselectedAnnotatorsArray)
